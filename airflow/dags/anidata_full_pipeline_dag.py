...
with DAG(
    ...
    bash_command=(
        ...
        "mkdir -p /opt/airflow/output && "
        "rm -f /opt/airflow/output/anime_gold.csv /opt/airflow/output/anime_gold.json /opt/airflow/output/feature_engineering_log_1.txt /opt/airflow/output/feature_engineering_log_2.txt; "
        # Run 1
        "rm -f /opt/airflow/output/anime_gold.csv /opt/airflow/output/anime_gold.json; "
        ...
        "if [ ! -s /opt/airflow/output/anime_gold.json ]; then echo FAIL > /opt/airflow/output/feature_engineering_status.txt; echo 'anime_gold.json missing/empty'; exit 1; fi; "
        ...
        "hash1_combined=$(echo \"$hash1_csv $hash1_json\" | sha256sum | awk '{print $1}'); "
        # Run 2
        "rm -f /opt/airflow/output/anime_gold.csv /opt/airflow/output/anime_gold.json; "
        ...
        "hash2_combined=$(echo \"$hash2_csv $hash2_json\" | sha256sum | awk '{print $1}'); "
        # Compare hashes
        "if [ \"$hash1_combined\" != \"$hash2_combined\" ]; then "
        "  echo FAIL > /opt/airflow/output/feature_engineering_status.txt; "
        "  echo \"Hash mismatch for feature engineering\"; "
        "  exit 1; "
        "fi; "
        # Final checks on run 2 logs / outputs
        "grep -q 'Feature engineering terminé !' /opt/airflow/output/feature_engineering_log_2.txt || "
        "(echo FAIL > /opt/airflow/output/feature_engineering_status.txt; echo 'Message de succès absent'; exit 1); "
